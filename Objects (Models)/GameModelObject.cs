...
namespace Othello_Project_1._0.Objects__Models_
{
    internal class GameModelObject
    {
        ...
        public void FlipsTokens(int Newx, int Newy, int dx, int dy) //checks if the next square is a oppositional white token (when blacks turn)
        {
            List<int[]> Opps = new List<int[]>
            {
            ...
            if (NEWX >= 0 && NEWX < NumberOfRows && NEWY >= 0 && NEWY < NumberOfColumns)
            {
                if (GameBoard[NEWX, NEWY] == OpponentTileState) 
                {
                    FlipsTokens(NEWX, NEWY, dx, dy); // flips next oppositional token in that direction
                }
